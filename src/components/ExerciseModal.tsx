    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-slate-800">Break Exercises</h2>
          <button
            onClick={onClose}
          ...
          {exercises.map((exercise, index) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div className={`${exercise.color} text-white p-3 rounded-xl flex-shrink-0`}>
                  {exercise.icon}
                </div>
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-slate-500 font-medium">{exercise.duration}s</span>
                  </div>
                  
                  <p className="text-slate-600 font-medium mb-2">{exercise.description}</p>
                  
                  {currentExercise === index && (
                    <div className="mt-3">
                      <p className="text-sm text-slate-700 mb-3">{exercise.instructions}</p>
                      {!completed[index] && (
                        <button
                          onClick={() => markComplete(index)}
                          className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-medium transition-colors duration-200 flex items-center gap-2"
                        >
        ...
        <div className="mt-6 pt-4 border-t border-slate-200">
          {allCompleted ? (
            <div className="text-center">
              ...
              <p className="text-green-700 font-semibold mb-4">Great job! You've completed all exercises.</p>
              <button
                onClick={onClose}
